...
class SinglePhase extends CardPanel {
    ...
    private IDeckOfCards removeSomeCardsIfGreaterThanSix(ICardStack cards) {
        ...
        if (cards.getList().size() > GUIConstants.MAX_SHOWN_CARDS) {
            min.addAll(cards.getList().subList(0, (GUIConstants.MAX_SHOWN_CARDS / 2)));
            min.addAll(cards.getList().subList(cards.getList().size() - (GUIConstants.MAX_SHOWN_CARDS / 2), cards.getList().size()));
        } else {
